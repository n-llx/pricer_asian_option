...
if(EXISTS "${CMAKE_CURRENT_LIST_DIR}/Qt6QuickDependencies.cmake")
    ...
    _qt_internal_suggest_dependency_debugging(Quick
        __qt_Quick_pkg ${CMAKE_FIND_PACKAGE_NAME}_NOT_FOUND_MESSAGE)
endif()
...
if (NOT QT_NO_CREATE_TARGETS AND Qt6Quick_FOUND)
    ...
    include("${CMAKE_CURRENT_LIST_DIR}/Qt6QuickAdditionalTargetInfo.cmake"
        OPTIONAL)
    include("${CMAKE_CURRENT_LIST_DIR}/Qt6QuickExtraProperties.cmake"
        OPTIONAL)
endif()
...
set(__qt_Quick_always_load_private_module ON)
set(__qt_Quick_save_QT_NO_PRIVATE_MODULE_WARNING ${QT_NO_PRIVATE_MODULE_WARNING})
set(QT_NO_PRIVATE_MODULE_WARNING ON)
include("${CMAKE_CURRENT_LIST_DIR}/Qt6Quick-build.cmake" OPTIONAL)
if (Qt6Quick_FOUND
        AND NOT Qt6QuickPrivate_FOUND
        AND NOT FALSE
        AND (
            __qt_Quick_always_load_private_module
    ...
    find_package(Qt6QuickPrivate "6.9.3" EXACT
        QUIET
        CONFIG
        PATHS
    ...
    if(NOT Qt6QuickPrivate_FOUND)
        get_property(Qt6QuickPrivate_warning_shown GLOBAL PROPERTY
            Qt6QuickPrivate_warning_shown
        )
        if(NOT Qt6QuickPrivate_warning_shown)
            message(VERBOSE
                "The private module package 'Qt6QuickPrivate' "
                "could not be found. It possibly needs to be installed separately with your "
                "package manager."
            )
            set_property(GLOBAL PROPERTY
                Qt6QuickPrivate_warning_shown ON
            )
        endif()
    endif()
endif()
set(QT_NO_PRIVATE_MODULE_WARNING ${__qt_Quick_save_QT_NO_PRIVATE_MODULE_WARNING})
unset(__qt_Quick_save_QT_NO_PRIVATE_MODULE_WARNING)
unset(__qt_Quick_always_load_private_module)
...
if (NOT QT_NO_CREATE_TARGETS AND Qt6Quick_FOUND)
    ...
    if(NOT Qt6Quick_DEFINITIONS)
        ...
        set(updated_defs "")
        foreach(def IN LISTS  Qt6Quick_DEFINITIONS)
        ...
        endforeach()
        set(Qt6Quick_DEFINITIONS "${updated_defs}")
        unset(updated_defs)
    endif()
    ...
    foreach(_module_dep ${_Qt6Quick_MODULE_DEPENDENCIES})
        ...
        list(APPEND Qt6Quick${_private_suffix}_INCLUDE_DIRS
             ${Qt6${_module_dep}_INCLUDE_DIRS})
        list(APPEND Qt6Quick${_private_suffix}_PRIVATE_INCLUDE_DIRS
             ${Qt6${_module_dep}_PRIVATE_INCLUDE_DIRS})
        if(_private_suffix)
            list(APPEND Qt6Quick_PRIVATE_INCLUDE_DIRS
                ${Qt6${_module_dep}_PRIVATE_INCLUDE_DIRS})
        endif()
        list(APPEND Qt6Quick${_private_suffix}_DEFINITIONS
             ${Qt6${_module_dep}_DEFINITIONS})
        list(APPEND Qt6Quick${_private_suffix}_COMPILE_DEFINITIONS
...
if (TARGET Qt6::Quick)
    ...
    if(NOT "Quick" IN_LIST QT_ALL_MODULES_FOUND_VIA_FIND_PACKAGE)
    ...
    if(NOT QT_NO_CREATE_VERSIONLESS_TARGETS)
        if(CMAKE_VERSION VERSION_LESS 3.18 OR QT_USE_OLD_VERSION_LESS_TARGETS)
            ...
            include("${CMAKE_CURRENT_LIST_DIR}/Qt6QuickVersionlessAliasTargets.cmake")
        endif()
    endif()
else()
    ...
    if(NOT DEFINED Qt6Quick_NOT_FOUND_MESSAGE)
        set(Qt6Quick_NOT_FOUND_MESSAGE
            "Target \"Qt6::Quick\" was not found.")
