...
if (NOT QT_NO_CREATE_TARGETS AND Qt6Sql_FOUND)
    ...
    include("${CMAKE_CURRENT_LIST_DIR}/Qt6SqlAdditionalTargetInfo.cmake"
        OPTIONAL)
    include("${CMAKE_CURRENT_LIST_DIR}/Qt6SqlExtraProperties.cmake"
        OPTIONAL)
endif()
...
set(__qt_Sql_always_load_private_module ON)
set(__qt_Sql_save_QT_NO_PRIVATE_MODULE_WARNING ${QT_NO_PRIVATE_MODULE_WARNING})
set(QT_NO_PRIVATE_MODULE_WARNING ON)
include("${CMAKE_CURRENT_LIST_DIR}/Qt6Sql-build.cmake" OPTIONAL)
if (Qt6Sql_FOUND
        AND NOT Qt6SqlPrivate_FOUND
        AND NOT FALSE
        AND (
            __qt_Sql_always_load_private_module
    ...
    find_package(Qt6SqlPrivate "6.9.3" EXACT
        QUIET
        CONFIG
        PATHS
    ...
    if(NOT Qt6SqlPrivate_FOUND)
        get_property(Qt6SqlPrivate_warning_shown GLOBAL PROPERTY
            Qt6SqlPrivate_warning_shown
        )
        if(NOT Qt6SqlPrivate_warning_shown)
            message(VERBOSE
                "The private module package 'Qt6SqlPrivate' "
                "could not be found. It possibly needs to be installed separately with your "
                "package manager."
            )
            set_property(GLOBAL PROPERTY
                Qt6SqlPrivate_warning_shown ON
            )
        endif()
    endif()
endif()
set(QT_NO_PRIVATE_MODULE_WARNING ${__qt_Sql_save_QT_NO_PRIVATE_MODULE_WARNING})
unset(__qt_Sql_save_QT_NO_PRIVATE_MODULE_WARNING)
unset(__qt_Sql_always_load_private_module)
...
if (NOT QT_NO_CREATE_TARGETS AND Qt6Sql_FOUND)
    ...
    if(NOT Qt6Sql_DEFINITIONS)
        ...
        set(updated_defs "")
        foreach(def IN LISTS  Qt6Sql_DEFINITIONS)
        ...
        endforeach()
        set(Qt6Sql_DEFINITIONS "${updated_defs}")
        unset(updated_defs)
    endif()
    ...
    foreach(_module_dep ${_Qt6Sql_MODULE_DEPENDENCIES})
        ...
        list(APPEND Qt6Sql${_private_suffix}_INCLUDE_DIRS
             ${Qt6${_module_dep}_INCLUDE_DIRS})
        list(APPEND Qt6Sql${_private_suffix}_PRIVATE_INCLUDE_DIRS
             ${Qt6${_module_dep}_PRIVATE_INCLUDE_DIRS})
        if(_private_suffix)
            list(APPEND Qt6Sql_PRIVATE_INCLUDE_DIRS
                ${Qt6${_module_dep}_PRIVATE_INCLUDE_DIRS})
        endif()
        list(APPEND Qt6Sql${_private_suffix}_DEFINITIONS
             ${Qt6${_module_dep}_DEFINITIONS})
        list(APPEND Qt6Sql${_private_suffix}_COMPILE_DEFINITIONS
...
if (TARGET Qt6::Sql)
    ...
    if(NOT "Sql" IN_LIST QT_ALL_MODULES_FOUND_VIA_FIND_PACKAGE)
    ...
    if(NOT QT_NO_CREATE_VERSIONLESS_TARGETS)
        if(CMAKE_VERSION VERSION_LESS 3.18 OR QT_USE_OLD_VERSION_LESS_TARGETS)
            ...
            include("${CMAKE_CURRENT_LIST_DIR}/Qt6SqlVersionlessAliasTargets.cmake")
        endif()
    endif()
else()
