...
\begin{titlepage}
    ...
    {\large Antoine BERGIER \par}
    {\large Nils LELORIEUX \par}
    {\large Alexandre ROGOJAN \par}
...
\begin{description}
    ...
    \item[\texttt{App/}] Encapsulates the application logic.
    \begin{itemize}
        \item \texttt{PricerApp.hpp/cpp}: Manages the application lifecycle, user input handling, and orchestration of the Monte Carlo simulation and plotting.
    \end{itemize}

    \item[\texttt{main/}] Application entry point.
    \begin{itemize}
        \item \texttt{main.cpp}: A lightweight wrapper that instantiates and runs the \texttt{PricerApp}.
    \end{itemize}
\end{description}
