...
\begin{titlepage}
    ...
    \vspace{0.5cm}
    {\Large \textbf{Professor:} Mrs. DUMITRESCU \par}
...
\begin{itemize}
    ...
    \item \textbf{Greeks Calculation}: Computation of sensitivities of real-life stocks (using live-data fetched from Yahoo): Delta, Vega, Gamma, Rho and Theta.
...
The project is designed to be fully portable and direclty executed on Windows and MacOS environments (it includes a portable compiler (MinGW) and all dependencies).

\bigskip
\newline
\noindent \textbf{On Windows:}
\begin{enumerate}
...
\bigskip
\newline
\noindent \textbf{On MacOS:}
\begin{enumerate}
    \item \textbf{Locate the Script}: Find `run\_project.sh` in the root directory.
    \item \textbf{Execute}: Open a new terminal inside the project's folder and run the following commands:
    \begin{lstlisting}[language=bash, frame=single, backgroundcolor=\color{gray!10}]
    > chmod +x run_project.sh
    > ./run_project.sh #to compile the project
    \end{lstlisting}
    \item \textbf{Workflow}:
    \begin{itemize}
        \item The script automatically checks for the compiler and sources.
...
All other pieces of code are external and are: a portable compiler, a portable plotter, and helpers to make the software run smoothly. 
...
\begin{itemize}
    \item \textbf{Compilation issues}: Configuring the build system to work portably and reliably across different Windows machines was challenging. This was resolved by including a local MinGW distribution and using a batch script for automatic path configuration.Some security issues were also encountered on the MacOS environment.
    ...
    \item \textbf{Optimization}: The code needed to be overhauled several times. Indeed, memory allocations were often times too numerous, requiring the use of pointers. The surfaces plotting generates for each point a Monte-Carlo simulation with, for each point (grid size of 50x50) N simulations containing many steps. 
    \item \textbf{Maths}: Optimizing the code required the use of maths to simplify calculations: the put surface is derived from the call surface using put/call parity for example. The delta, gamma and vega surfaces are derived from the call surface by finite difference derivation, thus slashing the number of Monte-Carlo simulations from 4 to 1 to output 4 graphs (put prices and 3 first greeks). However other greeks needed new Monte-Carlo simulations as they are derived from other parameters than spot price and volatility.
    \item \textbf{Live-Data fetching}: Acquiring real-world data about stocks is challenging. Most APIs are either expensive or unreliable or limited by their number of calls. We stuck with Yahoo as it provides basic price Data, from which we can derive historical volatility and calculate option prices. Although our software can't currently be calibrated to be used in the real-world (with the implied volatility for example), we remain consistent with the GBM model assumptions (constant volatility,...).
\end{itemize}
...
The architecture of this project was driven by three primary goals: \textbf{performance}, \textbf{portability} (so that it can run directly on any environment), and \textbf{simplicity}. We wanted a codebase that is straightforward to compile on any Windows machine while being efficient enough to run heavy Monte Carlo simulations.
