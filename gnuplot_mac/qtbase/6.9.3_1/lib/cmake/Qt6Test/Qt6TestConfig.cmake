...
if(EXISTS "${CMAKE_CURRENT_LIST_DIR}/Qt6TestDependencies.cmake")
    ...
    _qt_internal_suggest_dependency_debugging(Test
        __qt_Test_pkg ${CMAKE_FIND_PACKAGE_NAME}_NOT_FOUND_MESSAGE)
endif()
...
set(__qt_Test_always_load_private_module ON)
set(__qt_Test_save_QT_NO_PRIVATE_MODULE_WARNING ${QT_NO_PRIVATE_MODULE_WARNING})
set(QT_NO_PRIVATE_MODULE_WARNING ON)
include("${CMAKE_CURRENT_LIST_DIR}/Qt6Test-build.cmake" OPTIONAL)
if (Qt6Test_FOUND
        AND NOT Qt6TestPrivate_FOUND
        AND NOT FALSE
        AND (
            __qt_Test_always_load_private_module
    ...
    find_package(Qt6TestPrivate "6.9.3" EXACT
        QUIET
        CONFIG
        PATHS
    ...
    if(NOT Qt6TestPrivate_FOUND)
        get_property(Qt6TestPrivate_warning_shown GLOBAL PROPERTY
            Qt6TestPrivate_warning_shown
        )
        if(NOT Qt6TestPrivate_warning_shown)
            message(VERBOSE
                "The private module package 'Qt6TestPrivate' "
                "could not be found. It possibly needs to be installed separately with your "
                "package manager."
            )
            set_property(GLOBAL PROPERTY
                Qt6TestPrivate_warning_shown ON
            )
        endif()
    endif()
endif()
set(QT_NO_PRIVATE_MODULE_WARNING ${__qt_Test_save_QT_NO_PRIVATE_MODULE_WARNING})
unset(__qt_Test_save_QT_NO_PRIVATE_MODULE_WARNING)
unset(__qt_Test_always_load_private_module)
...
if (NOT QT_NO_CREATE_TARGETS AND Qt6Test_FOUND)
    ...
    if(NOT Qt6Test_DEFINITIONS)
        ...
        set(updated_defs "")
        foreach(def IN LISTS  Qt6Test_DEFINITIONS)
        ...
        endforeach()
        set(Qt6Test_DEFINITIONS "${updated_defs}")
        unset(updated_defs)
    endif()
    ...
    foreach(_module_dep ${_Qt6Test_MODULE_DEPENDENCIES})
        ...
        list(APPEND Qt6Test${_private_suffix}_INCLUDE_DIRS
             ${Qt6${_module_dep}_INCLUDE_DIRS})
        list(APPEND Qt6Test${_private_suffix}_PRIVATE_INCLUDE_DIRS
             ${Qt6${_module_dep}_PRIVATE_INCLUDE_DIRS})
        if(_private_suffix)
            list(APPEND Qt6Test_PRIVATE_INCLUDE_DIRS
                ${Qt6${_module_dep}_PRIVATE_INCLUDE_DIRS})
        endif()
        list(APPEND Qt6Test${_private_suffix}_DEFINITIONS
             ${Qt6${_module_dep}_DEFINITIONS})
        list(APPEND Qt6Test${_private_suffix}_COMPILE_DEFINITIONS
...
if (TARGET Qt6::Test)
    ...
    foreach(extra_cmake_include QtTestProperties.cmake)
    ...
    if(NOT "Test" IN_LIST QT_ALL_MODULES_FOUND_VIA_FIND_PACKAGE)
    ...
    if(NOT QT_NO_CREATE_VERSIONLESS_TARGETS)
        if(CMAKE_VERSION VERSION_LESS 3.18 OR QT_USE_OLD_VERSION_LESS_TARGETS)
            ...
            include("${CMAKE_CURRENT_LIST_DIR}/Qt6TestVersionlessAliasTargets.cmake")
        endif()
    endif()
else()

    set(Qt6Test_FOUND FALSE)
    if(NOT DEFINED Qt6Test_NOT_FOUND_MESSAGE)
        set(Qt6Test_NOT_FOUND_MESSAGE
            "Target \"Qt6::Test\" was not found.")

        if(QT_NO_CREATE_TARGETS)
            string(APPEND Qt6Test_NOT_FOUND_MESSAGE
                "Possibly due to QT_NO_CREATE_TARGETS being set to TRUE and thus "
                "${CMAKE_CURRENT_LIST_DIR}/Qt6TestTargets.cmake was not "
