...
\begin{itemize}
    ...
    \item \textbf{Teamwork and Collaboration:} One of the challenges during the project was coordinating the work across the team. Working simultaneously on the same codebase sometimes led to merge conflicts, which made the integration of different features a bit more time-consuming. Additionally, getting familiar with the code written by other team members required regular communication to make sure the overall architecture stayed consistent and clear for everyone.
    ...
    \item \textbf{Greek Sensitivities Calculation (Theta and Rho):} Computing $\Theta$ (Theta) and $\rho$ (Rho) using finite difference methods initially produced aberrant results.
    \begin{itemize}
        \item Regarding Theta, the difficulty lay in choosing the time step $dt$. A step that was too large introduced significant bias, while a step that was too small amplified the numerical noise inherent in Monte Carlo simulations.
        \item Regarding Rho, the low sensitivity of the price to interest rate variations made the calculation unstable when faced with simulation variance.
    \end{itemize}
    These issues were partially resolved by implementing variance reduction techniques—specifically using identical random seeds for perturbed paths. While improved, the precision of these two parameters remains more sensitive to noise than that of Delta or Vega.
\end{itemize}
